...
describe('ExpenseClassifierService', () => {
  ...
  it('xử lý đúng các title tiện ích ngắn như điện, nước', () => {
    expect(service.classify('Điện')).toBe('housing');
    expect(service.classify('Nước')).toBe('housing');
    expect(service.classify('Điện nước')).toBe('housing');
    expect(service.classify('Nước uống')).toBe('essential');
  });
